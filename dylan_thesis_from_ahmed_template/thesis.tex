...
General Purpose computing with Graphics Processing Units (GPGPU) is a method of computation that allows for many similar calculations to be performed simultaneously. Graphics Processing Units (GPU) allow a single basic computer to perform as well as a super computer cluster composed of only Central Processing Units (CPU), at a fraction of the cost.  As quantum computational chemistry scales very poorly for larger systems, the use of GPGPU could be a very effective tool for quantum chemists.

This thesis presents a program written in CUDA Fortran called cudaDFRATOM. cudaDFRATOM allows for the calculation and optimization of Well-Tempered Basis Sets (WTBS) at the four-component relativistic level of theory. Except for a few small initial calculations and control flow, all work is performed entirely on the GPU. 

When calculating the two-electron integrals, cudaDFRATOM uses a very effective method of utilizing as much of the resources of the GPU as possible and can calculate these integrals approximately 20 times faster than a similar program running on only a CPU. 
