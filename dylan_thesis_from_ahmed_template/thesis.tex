...
where $R_{nl}$ is called the radial equation and $Y^{m}_{l}(\theta,\phi)$ is called the angular equation. The radial equation has the form of
...
Y^{m}_{l}(\theta,\phi) = \epsilon\sqrt{\frac{(2l + 1)}{4\pi}\frac{(l-|m|)!}{(l+|m|)!}}P^{m}_{l}(cos\theta)e^{im\phi}
\end{equation}
...
\cmidrule(lr){1-1} \cmidrule(lr){2-4}
				...
				&	$m=0$	&	$m=0$	&	$m=\pm1$	\\
\midrule
1	&	$\left(\frac{1}{\pi a^{3}}\right)^{\frac{1}{2}}e^{\left(-\frac{r}{a}\right)}$			&	$-$	&	$-$	\\
\\
2	&	$\frac{1}{\sqrt{8\pi a^{3}}}\left(1-\frac{r}{2a}\right)e^{\left(-\frac{r}{2a}\right)}$	&	$\frac{1}{\sqrt{32\pi a^{3}}}\frac{r}{a}e^{\left(-\frac{r}{2a}\right)}cos\theta $	&	
$\mp\frac{1}{\sqrt{64\pi a^{3}}}\frac{r}{a}e^{\left(-\frac{r}{2a}\right)}sin\theta e^{\pm i \phi}$	\\
\bottomrule
\end{tabular}
\end{table}
...
As can be seen, changing the indexing of the electrons is equivalent to swapping the rows for those electrons, which changes the sign of the determinant. It is important to note that because observable quantities depend not the wavefunction, but the \textit{absolute square} of the wavefunction, altering the indexing of the electrons does not change the calculation of these observables. Consider the example of the electron density probability distribution shown below (note that $\psi$, $\alpha$, and $\beta$ are real in this example, but even if they weren't the results would not change).
...
[
\psi_{1}(1)^{2}\alpha(\omega_{1})^{2}\psi_{1}(2)^{2}\beta(\omega_{2})^{2} +
\psi_{1}(1)^{2}\beta(\omega_{1})^{2}\psi_{1}(2)^{2}\alpha(\omega_{2})^{2}
	\\
	&	-2
\psi_{1}(1)^{2}\alpha(\omega_{1})\beta(\omega_{1})\psi_{1}(2)^{2}\beta(\omega_{2})\alpha(\omega_{2})
...
\frac{1}{2}
\left[
|\psi_{1}(1)|^{2}|\psi_{1}(2)|^{2} + |\psi_{1}(1)|^{2}|\psi_{1}(2)|^{2}
\right]
\end{equation}
...
I will combine the first two terms into an operator I will call the $H^{\text{core}}$ operator ($h(i)$) and $v^{HF}(i)$ is the potential generated by all the electrons except the $i^{th}$ one. $v^{HF}(i)$ is composed of two different operators: the coulomb operator and the exchange operator. The coulomb operator is intuitive, it is simply the interaction electron 1 experiences when in a coulomb potential generated by electron 2
...
\psi_{u} \approx \sum^{K}_{i=1}C_{u,i}\Phi(\zeta_{u,i})
\end{equation}

where $\zeta_{u,i}$ is the $i^{th}$ adjustable parameters of the basis function $\Phi$ which is approximating the $u^{th}$ orbital, $C_{u,i}$ is the $i^{th}$ coefficient of $i^{th}$ basis function of the $u^{th}$ $\psi$, and $K$ is the total number of basis functions that make up the basis set. $C_{u,i}$ can intuitively be thought of as the ``percentage" of $\psi$ that $\Phi(\zeta_{u,i})$ makes up (this is not strictly speaking true, as $C_{u,i}$ can be greater than one or negative, but the general idea is there).
