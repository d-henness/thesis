...
The first rule is that the overall wavefunction must satisfy the Pauli exclusion principle\citethis. There are two components to this, the first is that two electrons can not occupy the same orbital if they have the same \textit{spin}, and the second is that the wavefunction must be antisymmetric with respect to the exchanging of electronic coordinates. Spin is actually a fourth quantum number, which I have neglected to talk about so far. Spin is a confusing property of some quantum particles have. No one really knows what is is, other than to say it has something to do with the angular momentum of the particle, and oddly enough that it in no way refers to the direction that the particle is spinning. It has been shown to have binary values, which I will refer to as up spin ($\uparrow$) and down spin ($\downarrow$). We can satisfy the requirements of the Pauli exclusion principal by using a Slater determinant\citethis, an example of which is shown in equation \ref{eq:slate_det}.
