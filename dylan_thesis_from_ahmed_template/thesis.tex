...
The first rule is our combination of \textit{trial} wavefunctions ($\psi(r)$) must meet the boundary conditions of the problem. For atoms and molecules, this means that the our collection of $\psi(r)$s must go to 0 as $r$ goes to infinity. This is easy enough to satisfy, we just saw one that does this in Equation \ref{eq:wavehydro}. The second rule is that the overall wavefunction must satisfy the Pauli exclusion principle.   

\section{Hartree-Fock SCF}
\notetodylan{rewrite this section so that it makes sense with what came before it.}
...
\textit{Step 2.} Optimize the basis sets and select the smallest basis set that is still below some minimum accuracy threshold. \newaddition{We chose a relative error of no greater than $5.0\times10^{-9}$ to numerical calculations where relative error is defined as $\frac{|(\text{optimized energy}) - (\text{numerical energy})|}{\text{numerical energy}}$}. The size of this basis set is s$(1:40)$p$(1:40)$d$(1:40)$f$(1:x_{f})$.
...
This process has a major drawback in that it requires a lot of computer power to run efficiently. But this is not really a problem if access to large computer clusters is available. \newaddition{Additionally, considering that calculations for the chain 1-8 can be run in parallel for different atoms, this optimization process is trivially parallel.} The advantages of this are it finds a very small basis set that is still accurate, and it is also completely automatable. If there are no problems with individual calculations not converging, the output files never even need to be manually examined!
...
The first problem arises due to warp divergence. Because the maximum values of $M$, $R$, and $S$ depend on $L$, $P$, and $Q$, different threads will have a different number of loops to complete than others. Because a streaming multiprocessor (SM) must finish the block is it currently working on before it can grab another, there is the possibility that most of the threads in a block are idling while waiting for others in the same block to finish. The second problem is that with this method, there will always be several blocks that have threads that remain idle throughout the block's runtime, no matter what. \newaddition{This can be seen more clearly in Figure \ref{fig:naivelayout}}. The third problem with this is that with the maximum values for $L$, $P$, and $Q$ available for a single atom, there might not even be enough combinations to completely fill the GPU. While all of these issues begin to disappear once the number of integrals to evaluate becomes large enough, we are still very much in the range where they are in play. Therefore a smarter algorithm had to be used.
...
{\newaddition{Two-Electron Integral Layout: 1D Blocks and Grid. Orange lines indicate the border of blocks. Notice that only the last block contains idle threads.}}
